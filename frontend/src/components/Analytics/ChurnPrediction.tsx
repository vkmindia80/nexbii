import React from 'react';
import { UserX } from 'lucide-react';

interface Props {
  dataSources: Array<{ id: string; name: string; type: string }>;
}

const ChurnPrediction: React.FC<Props> = ({ dataSources }) => {
  return (
    <div className="p-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <UserX className="w-6 h-6 text-blue-600" />
          Churn Prediction
        </h2>
        <p className="text-sm text-gray-600 mt-1">
          Predict customer churn with Random Forest models and identify high-risk customers
        </p>
      </div>
      <div className="bg-red-50 border border-red-200 rounded-lg p-8 text-center">
        <p className="text-red-800">Churn prediction interface - Coming soon!</p>
        <p className="text-sm text-red-600 mt-2">API endpoints ready. Use /api/analytics/churn-prediction</p>
      </div>
    </div>
  );
};

export default ChurnPrediction;